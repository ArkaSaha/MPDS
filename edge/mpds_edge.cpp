...
void* helper(void* arg)
{
	...
	for (int i = 1; i <= tdata->samples; i++)
	{
		...
		vector< vector<size_t> > cand = edge_exact(adj, tdata->n);
...
void mpds(int theta, int num_threads, size_t k, char* output, size_t n, vector< tuple<size_t, size_t, double> >& edges)
{
...
int main(int argc, char* argv[])
{
	...
	if (argc != 6)
	{
		cerr << "Usage : ./edge path-to-graph number-of-samples number-of-threads number-of-subgraphs path-to-output" << endl;
		return EXIT_FAILURE;
	}
	size_t n, u, v;
	double p;
	...
	fin.close();
	mpds(atoi(argv[2]), atoi(argv[3]), atoi(argv[4]), argv[5], n + 1, edges);
	return EXIT_SUCCESS;
}
