...
vector< vector<size_t> > core_reduce_star(unordered_map< size_t, unordered_set<size_t> >& graph, vector< vector<size_t> >& choose, size_t x)
{
	...
	for (auto& d : pat_deg)
		handles[d.first] = heap.push(node(d.first, d.second));
	vector< pair<size_t, size_t> > rem = vector< pair<size_t, size_t> >();
	rem.push_back(make_pair(numeric_limits<int>::max(), max_den));
	while (!heap.empty())
	{
		node nd = heap.top();
...
void mpds(int theta, int num_threads, size_t k, string pattern, char* output, vector< tuple<size_t, size_t, double> > edges)
{
...
int main(int argc, char* argv[])
{
	...
	fin.close();
	mpds(atoi(argv[2]), atoi(argv[3]), atoi(argv[4]), argv[5], argv[6], edges);
	return EXIT_SUCCESS;
}
